...
/**
 * Anthropic Claude provider implementation using claude-sonnet-4-20250514
 *
 * Benefits:
...
export class ClaudeProvider implements AIProvider {
     ...
     * Claude may wrap JSON in markdown code fences or add text around it.
     */
    private extractJson(text: string): string {
        ...
        // If response starts with '{', it's already clean JSON
        if (result.startsWith("{")) {
            return result;
        }

        // Try to find JSON object in the response (text before/after JSON)
