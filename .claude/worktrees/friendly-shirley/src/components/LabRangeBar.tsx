...
export function LabRangeBar({ value, referenceRange, unit, flag }: LabRangeBarProps) {
    ...
    const renderRangeBar = () => {
        ...
        const getMarkerColor = () => {
            if (flag === 'high') return 'bg-status-critical';
            if (flag === 'low') return 'bg-accent';
            if (flag === 'borderline') return 'bg-status-caution';
        ...
        return (
            <div className="relative w-full h-8 my-2">
                {/* Scale background */}
                <div className="absolute inset-0 bg-surface-warm rounded-full overflow-hidden">
                    {/* Normal range highlighted */}
                    <div
                        className="absolute h-full bg-status-normal-bg border-l border-r border-status-normal/30"
                        style={{
                ...
                {min !== null && max !== null && (
                    <>
                        <div
                            className="absolute -bottom-5 text-xs text-status-normal font-medium"
    ...
    return (
        <div className="mt-3 mb-6">
            <div className="text-xs text-text-secondary mb-1">Value on scale:</div>
            {renderRangeBar()}
        </div>
    );
}
