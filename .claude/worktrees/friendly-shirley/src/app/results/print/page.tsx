"use client";

import { useEffect, useState } from "react";
import Link from "next/link";
import { loadAnalysis } from "@/lib/persistence/analysisStorage";
import type { AnalysisApiResponse } from "@/lib/persistence/analysisStorage";
import {
    AnalysisResult,
    isLabReport,
    isDischargeInstructions,
} from "@/contracts/analysisSchema";

export default function PrintPage() {
    const [data, setData] = useState<AnalysisApiResponse | null>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        const analysis = loadAnalysis();
        setData(analysis);
        setLoading(false);
    }, []);

    const handlePrint = () => {
        window.print();
    };

    if (loading) {
        return (
            <div className="max-w-4xl mx-auto p-8">
                <p className="text-text-secondary">Loading analysis...</p>
            </div>
        );
    }

    if (!data || !data.result) {
        return (
            <div className="max-w-4xl mx-auto p-8">
                <div className="bg-status-caution-bg border border-status-caution/30 rounded-card p-6 text-center">
                    <h2 className="text-xl font-semibold text-status-caution mb-2">
                        No Analysis Data Found
                    </h2>
                    <p className="text-text-secondary mb-4">
                        No saved analysis was found. Please upload and analyze a document first.
                    </p>
                    <Link
                        href="/upload"
                        className="inline-block px-4 py-2 bg-accent text-text-inverse rounded-card hover:bg-accent-dark print:hidden"
                    >
                        Go to Upload
                    </Link>
                </div>
            </div>
        );
    }

    const { documentType, readingLevel, result, requestId } = data;
    const generatedTime = new Date().toLocaleString();

    return (
        <div className="max-w-4xl mx-auto p-8 bg-surface-card">
            {/* Print Button - Hidden in print mode */}
            <div className="mb-6 print:hidden flex gap-4">
                <button
                    onClick={handlePrint}
                    className="px-6 py-2 bg-accent text-text-inverse rounded-card hover:bg-accent-dark font-medium shadow-card"
                >
                    Print / Save as PDF
                </button>
                <Link
                    href="/results"
                    className="px-6 py-2 bg-surface-warm text-text-primary rounded-card hover:bg-surface font-medium"
                >
                    ← Back to Results
                </Link>
            </div>

            {/* Report Header */}
            <div className="border-b-2 border-accent-muted pb-6 mb-6">
                <h1 className="text-3xl font-bold text-text-primary mb-2">
                    MedLabLingo Analysis Report
                </h1>
                <div className="text-sm text-text-secondary space-y-1">
                    <p>
                        <span className="font-medium">Generated:</span> {generatedTime}
                    </p>
                    <p>
                        <span className="font-medium">Document Type:</span>{" "}
                        {documentType.replace("_", " ").replace(/\b\w/g, (l) => l.toUpperCase())}
                    </p>
                    <p>
                        <span className="font-medium">Reading Level:</span>{" "}
                        {readingLevel.charAt(0).toUpperCase() + readingLevel.slice(1)}
                    </p>
                    {requestId && (
                        <p>
                            <span className="font-medium">Request ID:</span>{" "}
                            <code className="bg-surface px-1 rounded text-xs">{requestId}</code>
                        </p>
                    )}
                </div>
            </div>

            {/* Patient Summary */}
            <section className="mb-8">
                <h2 className="text-2xl font-bold text-text-primary mb-4">Summary</h2>
                <p className="text-text-primary mb-4 leading-relaxed">
                    {result.patientSummary.overallSummary}
                </p>

                {result.patientSummary.keyTakeaways && result.patientSummary.keyTakeaways.length > 0 && (
                    <div>
                        <h3 className="text-lg font-semibold text-text-primary mb-2">Key Takeaways:</h3>
                        <ul className="list-disc ml-6 space-y-2 text-text-primary">
                            {result.patientSummary.keyTakeaways.map((takeaway, index) => (
                                <li key={index}>{takeaway}</li>
                            ))}
                        </ul>
                    </div>
                )}
            </section>

            {/* Questions for Doctor */}
            {result.questionsForDoctor && result.questionsForDoctor.length > 0 && (
                <section className="mb-8">
                    <h2 className="text-2xl font-bold text-text-primary mb-4">
                        Questions to Ask Your Doctor
                    </h2>
                    <ol className="list-decimal ml-6 space-y-2 text-text-primary">
                        {result.questionsForDoctor.map((question, index) => (
                            <li key={index}>{question}</li>
                        ))}
                    </ol>
                </section>
            )}

            {/* Lab Report Section */}
            {isLabReport(result) && result.labsSection && (
                <section className="mb-8">
                    <h2 className="text-2xl font-bold text-text-primary mb-4">Lab Results</h2>

                    {result.labsSection.overallLabNote && (
                        <div className="bg-accent-light border border-accent-muted rounded-card p-4 mb-4">
                            <p className="text-sm text-accent-dark">
                                <span className="font-medium">Overall Note: </span>
                                {result.labsSection.overallLabNote}
                            </p>
                        </div>
                    )}

                    {result.labsSection.labs && result.labsSection.labs.length > 0 ? (
                        <table className="w-full border-collapse border border-accent-muted text-sm">
                            <thead className="bg-surface">
                                <tr>
                                    <th className="border border-accent-muted px-4 py-2 text-left font-semibold text-text-primary">
                                        Test Name
                                    </th>
                                    <th className="border border-accent-muted px-4 py-2 text-left font-semibold text-text-primary">
                                        Value
                                    </th>
                                    <th className="border border-accent-muted px-4 py-2 text-left font-semibold text-text-primary">
                                        Reference Range
                                    </th>
                                    <th className="border border-accent-muted px-4 py-2 text-left font-semibold text-text-primary">
                                        Flag
                                    </th>
                                    <th className="border border-accent-muted px-4 py-2 text-left font-semibold text-text-primary">
                                        Explanation
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {result.labsSection.labs.map((lab, index) => (
                                    <tr key={index} className={index % 2 === 0 ? "bg-surface-card" : "bg-surface"}>
                                        <td className="border border-accent-muted px-4 py-2 font-medium text-text-primary">
                                            {lab.name}
                                            {lab.importance === "high" && (
                                                <span className="ml-2 text-xs text-status-critical font-bold">
                                                    [HIGH PRIORITY]
                                                </span>
                                            )}
                                        </td>
                                        <td className="border border-accent-muted px-4 py-2 text-text-primary">
                                            {lab.value}
                                            {lab.unit && ` ${lab.unit}`}
                                        </td>
                                        <td className="border border-accent-muted px-4 py-2 text-text-primary">
                                            {lab.referenceRange || "N/A"}
                                        </td>
                                        <td className="border border-accent-muted px-4 py-2 font-semibold text-text-primary">
                                            {lab.flag.toUpperCase()}
                                        </td>
                                        <td className="border border-accent-muted px-4 py-2 text-xs text-text-secondary">
                                            {lab.explanation}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    ) : (
                        <p className="text-text-secondary">No lab values were parsed from this document.</p>
                    )}
                </section>
            )}

            {/* Discharge Instructions Section */}
            {isDischargeInstructions(result) && result.dischargeSection && (
                <div className="space-y-8">
                    {/* Home Care Steps */}
                    {result.dischargeSection.homeCareSteps && result.dischargeSection.homeCareSteps.length > 0 && (
                        <section>
                            <h2 className="text-2xl font-bold text-text-primary mb-4">
                                Home Care Instructions
                            </h2>
                            <ul className="space-y-2">
                                {result.dischargeSection.homeCareSteps.map((step, index) => (
                                    <li key={index} className="flex items-start">
                                        <span className="mr-3 text-accent font-bold">✓</span>
                                        <span className="text-text-primary">{step}</span>
                                    </li>
                                ))}
                            </ul>
                        </section>
                    )}

                    {/* Medications */}
                    {result.dischargeSection.medications && result.dischargeSection.medications.length > 0 && (
                        <section>
                            <h2 className="text-2xl font-bold text-text-primary mb-4">Medications</h2>
                            <div className="space-y-4">
                                {result.dischargeSection.medications.map((med, index) => (
                                    <div key={index} className="border border-accent-muted rounded-card p-4">
                                        <h3 className="font-bold text-lg text-text-primary mb-2">
                                            {med.name}
                                        </h3>
                                        <div className="space-y-2 text-sm">
                                            <p>
                                                <span className="font-medium">Purpose:</span> {med.purposePlain}
                                            </p>
                                            <p>
                                                <span className="font-medium">How to take:</span> {med.howToTakeFromDoc}
                                            </p>
                                            {med.cautionsGeneral && (
                                                <p className="bg-status-caution-bg border border-status-caution/30 p-2 rounded-card">
                                                    <span className="font-medium">Important:</span> {med.cautionsGeneral}
                                                </p>
                                            )}
                                        </div>
                                    </div>
                                ))}
                            </div>
                        </section>
                    )}

                    {/* Follow-Up */}
                    {result.dischargeSection.followUp && result.dischargeSection.followUp.length > 0 && (
                        <section>
                            <h2 className="text-2xl font-bold text-text-primary mb-4">Follow-Up Care</h2>
                            <ul className="list-disc ml-6 space-y-2 text-text-primary">
                                {result.dischargeSection.followUp.map((item, index) => (
                                    <li key={index}>{item}</li>
                                ))}
                            </ul>
                        </section>
                    )}

                    {/* Warning Signs */}
                    {result.dischargeSection.warningSignsFromDoc && result.dischargeSection.warningSignsFromDoc.length > 0 && (
                        <section className="bg-status-caution-bg border-2 border-status-caution/40 rounded-card p-4">
                            <h2 className="text-2xl font-bold text-status-caution mb-4">
                                Warning Signs
                            </h2>
                            <ul className="list-disc ml-6 space-y-2 text-text-primary">
                                {result.dischargeSection.warningSignsFromDoc.map((sign, index) => (
                                    <li key={index}>{sign}</li>
                                ))}
                            </ul>
                        </section>
                    )}

                    {/* Red Flags */}
                    {result.dischargeSection.generalRedFlags && result.dischargeSection.generalRedFlags.length > 0 && (
                        <section className="bg-status-critical text-text-inverse rounded-card p-4">
                            <h2 className="text-2xl font-bold text-text-inverse mb-4">
                                Seek Immediate Care If:
                            </h2>
                            <ul className="list-disc ml-6 space-y-2 text-text-inverse font-medium">
                                {result.dischargeSection.generalRedFlags.map((flag, index) => (
                                    <li key={index}>{flag}</li>
                                ))}
                            </ul>
                        </section>
                    )}
                </div>
            )}

            {/* Safety Disclaimer */}
            {result.meta && result.meta.safety && (
                <section className="mt-8 pt-6 border-t-2 border-accent-muted">
                    <h2 className="text-xl font-bold text-text-primary mb-4">
                        Important Safety Information
                    </h2>

                    {result.meta.safety.disclaimer && (
                        <div className="bg-surface border border-accent-muted rounded-card p-4 mb-4">
                            <p className="text-sm text-text-primary">
                                <span className="font-semibold">Disclaimer: </span>
                                {result.meta.safety.disclaimer}
                            </p>
                        </div>
                    )}

                    {result.meta.safety.emergencyNote && (
                        <div className="bg-status-critical-bg border-2 border-status-critical rounded-card p-4 mb-4">
                            <p className="text-sm font-semibold text-status-critical">
                                {result.meta.safety.emergencyNote}
                            </p>
                        </div>
                    )}

                    {result.meta.safety.limitations && result.meta.safety.limitations.length > 0 && (
                        <div className="bg-surface border border-accent-muted rounded-card p-4">
                            <p className="text-sm font-semibold text-text-primary mb-2">Limitations:</p>
                            <ul className="list-disc ml-5 space-y-1 text-sm text-text-secondary">
                                {result.meta.safety.limitations.map((limitation, index) => (
                                    <li key={index}>{limitation}</li>
                                ))}
                            </ul>
                        </div>
                    )}
                </section>
            )}

            {/* Footer */}
            <footer className="mt-8 pt-6 border-t border-accent-muted text-center text-xs text-text-muted">
                <p>Generated by MedLabLingo - Educational Use Only</p>
                <p className="mt-1">This is not a substitute for professional medical advice.</p>
            </footer>
        </div>
    );
}
