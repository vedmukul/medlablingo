...
/**
 * Open Google Calendar directly to add all medication reminders.
 * Opens one tab per medication (Google Calendar URL API supports one event per link).
 */
export function openGoogleCalendarMeds(meds: MedicationEvent[]) {
    ...
    for (const med of meds) {
        ...
        // Use the first scheduled hour for the event
        const start = new Date(today);
        start.setHours(hours[0], 0, 0, 0);
        const end = new Date(start);
        end.setMinutes(15);

        const timingNote = hours.length > 1
            ? ` (${hours.map(h => `${h > 12 ? h - 12 : h}${h >= 12 ? 'PM' : 'AM'}`).join(', ')})`
            : "";

        const url = buildGoogleCalendarURL({
            title: `💊 ${med.name}${timingNote}`,
            details,
            start,
            end,
